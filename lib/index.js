...
module.exports = {
    ...
    createProxy: function createProxy(upstreams, options) {
        options = options || {};
        if (typeof(options) !== 'object')
            throw new TypeError('options (object) required');
        ...
        var opts = {
            hook_before_query: options.before,
            hook_after_query: options.after,
            hook_before_callback: options.before_cb,
            hook_after_callback: options.after_cb,
            name: options.name || 'named',
            log: options.log || bunyan.createLogger({
                name: 'named',
                level: 'warn',
        ...
        return (new Proxy(upstreams, opts));
    },
